...
class CDbCommand extends CComponent
{
    ...
    public function execute($params=array())
    {
        ...
        try
        {
            ...
            $table = "";
            if(preg_match_all('/((FROM|INTO|UPDATE) `([a-zA-Z_]*)`)/', $this->getText(), $matches)) {
                $tables = array_unique($matches[3]);
                $table = $tables[0];
            }

            CDbCacheManager::instance()->tableUpdated($table);
    ...
    private function queryInternal($method,$mode,$params=array())
    {
		...
		*/
        ...
        $entry = new CDbCacheEntry($this->getText(), array_merge($this->_paramLog,$params));
        $cache = CDbCacheManager::instance()->loadFromCache($entry);
        if($cache != null)
            return $cache;
        ...
        try
        {
            ...
            $entry->setTime($timeDiff);
            CDbCacheManager::instance()->save($entry, $result);
            return $result;
        }
        catch(Exception $e)
        {
